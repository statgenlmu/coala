...
test_that('Creation of simple mutation features works', {
  ...
  expect_equal(feat$get_table()$parameter[1], '5')
})
...
test_that('Creation of finite sites models works', {
  ...
  expect_equal(feat$get_table()$parameter, c('5', 'HKY', '2'))
  feat <- feat_mutation(par_const(5), model='F84', tstv_ratio = 2)
  expect_equal(feat$get_table()$parameter, c('5', 'F84', '2'))
  ...
  expect_equal(feat$get_table()$parameter,
  ...
  expect_equal(feat$get_table()$parameter, c('5', 'GTR', 1:6))
