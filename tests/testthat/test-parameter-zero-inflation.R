...
test_that("Parameters with variation can be initialized", {
  set.seed(114422)
  par <- par_zero_inflatation(par_const(5), 0.20)
  ...
  par <- par_zero_inflatation(5, .2)
  expect_that(par$eval(), is_a("numeric"))
  par <- par_zero_inflatation("5", .2)
  expect_that(par$eval(), is_a("numeric"))
  par <- par_zero_inflatation(par_named("x"), .2)
  x <- 100
  expect_that(par$eval(), is_a("numeric"))

  par <- par_zero_inflatation(5, "a")
  a <- .1
  expect_that(par$eval(), is_a("numeric"))
  par <- par_zero_inflatation(5, "a")
  a <- .1
...
test_that("Adding a parameter with variation to a model works", {
  ...
  model <- coal_model(5) + par_zero_inflatation(5, 100)
  expect_equal(length(get_parameter(model)), 0)
  expect_true(has_variation(model))

  model <- coal_model(5) + par_zero_inflatation(par_named("x"), 100)
