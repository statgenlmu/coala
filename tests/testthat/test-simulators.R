...
for (simulator in progs) {
  ...
  test_that(paste(name, "can simulate segsites"), {
    ...
    result <- simulator$simulate(model, 2, "-t 5")
  ...
  test_that(paste(name, "can simulate trees"), {
    ...
    result <- simulator$simulate(model, 2, "-T")
    ...
    result <- simulator$simulate(model, 2, "-r 10 100 -T")
    expect_equal(length(result$trees), 2)
    expect_that(result$trees[[1]], is_a("character"))
    expect_that(result$trees[[2]], is_a("character"))
  ...
  test_that(paste(name, "simulations are reproducible"), {
    model <- coal_model(10, 3, 100) + sumstat_trees() + sumstat_seg_sites()

    set.seed(17); result1 <- simulator$simulate(model, 3, "-r 1 100 -t 5 -T")
    set.seed(17); result2 <- simulator$simulate(model, 3, "-r 1 100 -t 5 -T")
