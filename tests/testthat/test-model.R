...
test_that("test.dm.getSummaryStatistic", {
  expect_equal(dm.getSummaryStatistics(model_theta_tau()),  as.factor("jsfs"))
  expect_equal(length(dm.getSummaryStatistics(model_theta_tau())), 1)
  expect_true(dm.getSummaryStatistics(model_grps(), 1) == "jsfs")
  expect_equal(length(dm.getSummaryStatistics(model_grps(), 1)),  1)
  expect_true(dm.getSummaryStatistics(model_grps(), 2) == "jsfs")
  expect_equal(length(dm.getSummaryStatistics(model_grps(), 2)), 1)
})
...
test_that("test.generateGroupModel", {
  ...
  expect_equal(dm@sum.stats, model_theta_tau()@sum.stats)
  ...
  sum.stats <- model_theta_tau()@sum.stats
...
test_that("test.getGroups", {
  expect_equal(dm.getGroups(model_theta_tau()), 1)
  dm <- dm.addLocus(model_theta_tau(), 23, 10, group = 1)
...
test_that("test.getSampleSize", {
  expect_equal(dm.getSampleSize(model_theta_tau()), c(10L, 15L))
})
...
test_that('locus length matrix generations works', {
  ...
  expect_equal(dm.getLociLengthMatrix(model_theta_tau()),
...
test_that('getting the available Populations works', {
  ...
  expect_equal(getPopulations(model_theta_tau()), 1:2)
  expect_equal(getPopulations(model_hky()), 1:3)
})
