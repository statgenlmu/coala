...
primarily consists of a `calculate` function that -- well -- calculates the
...
The calculate function must have four arguments. It is called after the 
simulation with the following arguments:

- First, a list of segregating sites, were each entry of the list contains
  the segregating sites for a locus.
...
Only the model is always passed to `calculate`. The other three inputs are
generated on demand and have to be requested by the statistic. To do so,
set the private variable `req_segsites`, `req_trees` or `req_files`, 
respectively, to `TRUE`. Arguments not request can be present if they are 
...
argument request. All the summary stastic does is to return the unmodified 
segregating sites.
...
essitial when defining your own constructors! See the `?sumstat_class` for
further details.
...
an [issue](https://github.com/statgenlmu/coala/issues/new) in coalas bug tracker
...
statistics. It should throw an error with `stop()` if it gets a model that is
not supported.
