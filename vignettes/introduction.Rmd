...
vignette: >
  %\VignetteIndexEntry{A introduction to coala}
  %\VignetteEngine{knitr::rmarkdown}
  %\usepackage[utf8]{inputenc}
---

Coalescent simulation referrers to the idea of simulating the evolution of 
...
and must explicitely be activated. This is described in the `installation`
vignette:
...
In this introduction we'll stick to using `scrm`.

# Creating a model
In order to conduct simulations, we first need to specify a model of
evolution that is used for the simulations. The function `coal_model` 
...
Simulated is one genetic locus for 3 haplotypes sampled from the population. 
...
If we a model consisting of multiple populations, we need to state the sample 
...
create a model of three populations, which a symmetric migration rate of `0.5`
between them. View backwards in time, population 3 merges into population 2
...
repesents two speciation events with migration going on afterwards. At time `0`
five haploids of population 1 and two ones of population 2 are sampled. Please
...
### Named Parameters
Named parameters values can be specified in the simulation command. If we want
to simulate a model for different values of the mutation rate, when can use
a named parameter:
...
sets a possible range from _0.1_ to _5_ for the mutation rate. The acctual
rate is given in `simulate` just as with named parameters.
